...
(defun run-vacuum (&key (agent (reactive-vacuum-agent 'A))
  ...
  (if gui
      (progn
	(agent-gui:init-gui (get-dirt-coords (grid-environment-objects env))
			    (get-obstacle-coords (grid-environment-objects env))
			    (list x y))))
  (run-eval-environment env :max-steps max-steps :display display :gui gui)
  (if gui
      (agent-gui:destroy-gui)))

(defun get-dirt-coords (obj-list)
  (cond
   ((endp obj-list) '())
   ((equal (object-name (first obj-list)) "*")
    (cons (object-loc (first obj-list))
	  (get-dirt-coords (rest obj-list))))
   (t (get-dirt-coords (rest obj-list)))))

(defun get-obstacle-coords (obj-list)
  (cond
   ((endp obj-list) '())
   ((equal (object-name (first obj-list)) "#")
    (cons (object-loc (first obj-list))
	  (get-obstacle-coords (rest obj-list))))
   (t (get-obstacle-coords (rest obj-list)))))
...
(defun make-vacuum-world (&key (x-size 10) (y-size x-size) (dirt 0.2)
...
(defun random-vacuum-agent (name)
  "Avery stupid agent: ignore percept and choose a random action."
  (vacuum-agent
   name
   #'(lambda (percept)
